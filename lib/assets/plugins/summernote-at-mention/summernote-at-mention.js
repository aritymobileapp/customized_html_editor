  ...
  $.extend($.summernote.plugins, {
    summernoteAtMention: function(context) {
      ...
      /*this.handleSelection = () => {
        ...
        // Create a new <a> tag for the selected suggestion
        const linkHTML = `<a href="#" class="mention-link">${newWord}</a>`;
        ...
        selectionPreserver.restore(absoluteIndex + linkHTML.length + 1);
      ...
      this.handleSelection = () => {
        ...
        // Create a new <a> tag for the selected suggestion
//        const linkHTML = `<a href="#" class="mention">@${newWord}</a>`;
        const linkHTML = `<a href="javascript:void(0)" onclick="mentionUser(this)">@{newWord}</a>`;

        // We need to get the container of the current selection and replace the word with the link
        const anchorNode = selection.anchorNode;
        const parentNode = anchorNode.parentNode;

        // Use insertAdjacentHTML to insert the link at the appropriate place in the parent node
        parentNode.innerHTML =

          linkHTML +
          " " +
          parentNode.innerHTML.slice(absoluteIndex + word.length);
