...
import 'package:customized_html_editor/src/html_editor_controller_unsupported.dart' as unsupported;
...
class HtmlEditorController extends unsupported.HtmlEditorController {
  ...
  void clearFocus() {
    throw Exception(
        'Flutter Web environment detected, please make sure you are importing package:customized_html_editor/html_editor.dart and check kIsWeb before calling this method.');
  }
  ...
  void addNotification(String html, NotificationType notificationType) {
    if (notificationType == NotificationType.plaintext) {
      ...
      _evaluateJavascriptWeb(data: {'type': 'toIframe: addNotification', 'html': html, 'alertType': 'alert alert-${describeEnum(notificationType)}'});
    }
    recalculateHeight();
  }
  ...
  void _evaluateJavascriptWeb({required Map<String, Object?> data}) async {
    if (kIsWeb) {
      ...
      throw Exception('Non-Flutter Web environment detected, please make sure you are importing package:customized_html_editor/html_editor.dart');
    }
  }
}
